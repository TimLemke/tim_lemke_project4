...
worldEatsApp.continentChoice = function (continentSelection) {
  ...
  selectedCuisines.forEach(function (cuisineName) {
    $('.buttonContainer').append(
      `<a href="#cuisineResults"><button class="cuisineOption animated flipInX" data-id=${cuisineName.id}>${cuisineName.name}</button></a>`
    );
  });
...
worldEatsApp.displayResults = function (cuisineResults) {
  ...
  cuisineResults.forEach(function (restaurant) {
    ...
    $('.results').append(
      `<div class="resultItem animated flipInX"><h4>${restaurant.name}</h4><p>${restaurant.location.address}</p><img class="thumbnail" src=${thumbnail}><a href=${restaurant.url} target="_blank">Get more info here!</a></div>`
    );
  });
